...
estimators <- function(Delta, moderator_var){ # , control_vars = control_vars
  ...
  lci_improved <- fit_wcls_improved$beta_hat - t_quantile * fit_wcls_improved$beta_se_adjusted
  rci_improved <- fit_wcls_improved$beta_hat + t_quantile * fit_wcls_improved$beta_se_adjusted
  
  re <- (fit_wcls$beta_se_adjusted/fit_wcls_new$beta_se_adjusted)^2
  re_improved <- (fit_wcls$beta_se_adjusted/fit_wcls_improved$beta_se_adjusted)^2
  ...
  if (is.null(moderator_var)){
    ...
    results <- rbind(results,
                     c(Delta, fit_wcls_improved$beta_hat, fit_wcls_improved$beta_se_adjusted,
                       rci_improved, lci_improved, "pd_IPW2", "beta0", re_improved))
  }else{
    ...
    results <- rbind(results,
                     c(Delta, fit_wcls_improved$beta_hat[1], fit_wcls_improved$beta_se_adjusted[1],
                       rci_improved[1], lci_improved[1], "pd_IPW2", "beta0", re_improved[1]))
    results <- rbind(results,
    ...
    results <- rbind(results,
                     c(Delta, fit_wcls_improved$beta_hat[2], fit_wcls_improved$beta_se_adjusted[2],
                       rci_improved[2], lci_improved[2], "pd_IPW2", "beta1", re_improved[2]))
...
for (moderator_var in moderator_vars) {
  ...
  saveRDS(results, paste0("drinkless_moderator=", moderator_var, ".RDS"))
}
