...
dgm_baserate <- function(sample_size, total_T, Delta, gamma, prob_a = 0.2) {
  ...
  for (t in 1:total_T) {
    ...
    dta$prob_R_0[row_index] <- (0.5*gamma)^((1.5-0.5*dta$S[row_index])/Delta)
  ...
  for (i_ss in 1:length(sample_sizes)) {
    ...
    result <- foreach(isim = 1:nsim, .combine = "c") %dorng% {
      ...
      dta <- data_generating_process(sample_size, total_T, 
                                     Delta = Delta, gamma = gamma, prob_a = 0.2)
...
#saveRDS(efficiency_table, file = "../kaggle/working/efficiency_table_baserate_fixedZt.RDS")
#saveRDS(result_table, file = "../kaggle/working/result_table_baserate_fixedZt.RDS")
saveRDS(efficiency_table, "efficiency_500.RDS")
saveRDS(result_table, "result_500.RDS")
...
for(q in (1:9)/10){
  L =  q *(1 - ((1-q)/(1-0.2))^7)/ (1 - ((1-q)/(1-0.2)))
  print((1 - (1 - q)^7)/ (L * (1 - 0.2)^7))
}
