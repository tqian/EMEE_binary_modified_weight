...
saveRDS(result_df_collected, file = "result_simulation_withmod.RDS")
...
result_df_collected <- readRDS("result_simulation_efficiency(delta = 3, beta_1 = 0.2, 2-parameter).RDS")
...
result_df_collected <- result_df_collected[,c(1,2,3,5,7,9,11,4,6,8,10,12)]

colnames(result_df_collected) <- 
  c("Estimator", "Sample Size",rep(c("Bias","SD","RMSE","CP(unadj)", "CP(adj)"),2))

result_df_collected[,c(3,4,5,8,9,10)] <- round(result_df_collected[,c(3,4,5,8,9,10)],3)
result_df_collected[,c(6,7,11,12)] <- round(result_df_collected[,c(6,7,11,12)],2)
