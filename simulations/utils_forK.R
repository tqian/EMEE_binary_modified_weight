weighted_centered_least_square_forK_new <- function(
  dta,
  id_varname,
...
)
{
  ...
  if (Delta == 1){
    ...
    for (i in 1:sample_size){
      ...
      dta_perid$cap_k <- ifelse(dta_perid$k <= Kref, dta_perid$k, Kref)
  ...
  # for each userid, pick out A_{t+1} ... A_{t+\delta-1} corresponds to each t 
  WCLS_weight_withDelta <- ifelse(A, p_t_tilde / p_t, (1 - p_t_tilde) / (1 - p_t)) * WCLS_weight_inverseProb
  ...
  estimating_equation <- function(theta) {
    ...
    for (i in 1:p) {
      ...
      ef[q + i] <- sum( weight * residual * avail * WCLS_weight_withDelta * cA_tilde * Xdm[, i]) 
    }
...
weighted_centered_least_square_forK <- function(
  dta,
  id_varname,
  decision_time_varname,
  treatment_varname,
...
)
{
  ...
  if (Kref == 0){
    ...
    for (i in 1:sample_size){
      ...
      for (j in 1:dta_timepoint) {
        WCLS_weight_inverseProb <- c(WCLS_weight_inverseProb,prod(inverseProb_numerator[(j+1):(j+Kref)]/
                                                                    inverseProb_denominator[(j+1):(j+Kref)]))
      }
    }
  }
  # for each userid, pick out A_{t+1} ... A_{t+\delta-1} corresponds to each t 
