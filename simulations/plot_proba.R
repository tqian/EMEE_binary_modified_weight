# code to generate plot for efficiency vs various treatment probability in the paper
...
for (prob_a in (1:8)/10) {
    ...
    for (i_ss in 1:length(sample_sizes)) {
        ...
        result <- foreach(isim = 1:nsim, .combine = "c") %dorng% {
            ...
            dta <- data_generating_process(sample_size, total_T, Delta, prob_a)
...
ggplot(efficiency_table) +
    geom_line(aes(x = `Prob_a(1)`, y = efficiency_means)) +
    geom_point(aes(x = `Prob_a(1)`, y = efficiency_means)) +
    theme_bw() +
    ylab("Relative Efficiency") +
    xlab("Randomization Probability of Treatment") +
